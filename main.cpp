...
//Todo's Content
vector<TextBox> Add_Task(1);
Button List_Back;
void Todo_Screen() {
    Add_Task[0].setType(1);
    List_Back.createButton(Renderer, "anh/back.png", 0, 0, 50, 50);
}
void Render_Add_Task () {
    Background.render( Renderer, 0, 0);
    Add_Task[0].render( Renderer, "+ Add a task: ", (SCREEN_WIDTH - Add_Task[0].getTag().getW() - 600) / 2, SCREEN_HEIGHT / 4.0, text_color, Content_Font);
        ...
        if (ev.type == SDL_QUIT) {
            ...
            case List:
                    if (ev.type == SDL_MOUSEMOTION) {
                        ...
                        if (Add_Task[0].is_mouse_inside(ev.button.x, ev.button.y) == true) {
                            Add_Task[0].setState(true);
                            pos = 0;
                            cout << pos << endl;
                        }
                        else {
                            Add_Task[0].setState(false);
                        }
                        if (List_Back.isInside(ev.button.x, ev.button.y)) {
                        ...
                        if (ev.key.keysym.sym == SDLK_BACKSPACE && Add_Task[pos].getInputText().length() > 0) {
                            if (Add_Task[pos].getState() == true) {
                                Add_Task[pos].inputPop();
                            }
                        }
                        else if (ev.key.keysym.sym == SDLK_c && SDL_GetModState() & KMOD_CTRL) {
                            SDL_SetClipboardText(Add_Task[pos].getInputText().c_str());
                        }
                        else if (ev.key.keysym.sym == SDLK_v && SDL_GetModState() & KMOD_CTRL) {
                            Add_Task[pos].setInputText(SDL_GetClipboardText());
                        }
                    }
                    else if (ev.type == SDL_TEXTINPUT) {
                        if (!(SDL_GetModState() & KMOD_CTRL && (ev.text.text[0] == 'c' || ev.text.text[0] == 'C' || ev.text.text[0] == 'v' || ev.text.text[0] == 'V'))) {
                            if (Add_Task[pos].getState() == true) {
                                Add_Task[pos].updateInputText(ev.text.text);
                            }
                        }
                    }
                break;
            ...
            while (quit == false) {
                ...
                if (Current == Start) {
                    ...
                    for (int i = 0; i < SignUp_Textbox.size(); i++) {
                     ...
                     if (Add_Task[0].getInputText().length() != 0) {
                            Add_Task[0].renderInputText(Renderer, Content_Font, text_color);
                        }
                }
